    ...
    while len(ConList) > 0:
        ...
        if len(Icon) == 1:
            ...
            for icom in range(len(IndCommon)):
                Pos = sorted([i for i, x in enumerate(
                    Index[Icon[0]]) if x == IndCommon[icom]])
                Tensor[Icon[0]] = np.trace(
                    Tensor[Icon[0]], axis1=Pos[0], axis2=Pos[1])
            ...
            A = np.tensordot(
                Tensor[Icon[0]], Tensor[Icon[1]], (Pos[0], Pos[1]))
...
def Mps_QRP(UL, T, icheck=0):
    ...
    if icheck == 1:
        ...
        A = np.tensordot(np.conj(Tnew), Tnew,
                         (range(0, rankT-1), range(0, rankT-1)))
...
if __name__ == "__main__":
    ...
    if Test['SplitSvd'] == 1:
        ...
        for Dcut in range(1, Dfull+1):
            for iweight in range(2):
                U, S, V, Dc = SplitSvd_Lapack(
                    ST, Dcut, iweight, mode='C', icheck=1)
    ...
    if Test['Mps_QRP'] == 1:
        ...
        for D1, D2, D3 in itertools.product(range(3, 5), range(3, 5), range(3, 5)):
            D = [D1, D2, D3]
            T0 = np.random.rand(D[0], D[1], D[2]) + 1j * \
                np.random.rand(D[0], D[1], D[2])
        ...
        for D1, D2, D3, D4 in itertools.product(range(3, 5), range(3, 5), range(3, 5), range(3, 5)):
            D = [D1, D2, D3, D4]
            T0 = np.random.rand(D[0], D[1], D[2], D[3]) + \
                1j*np.random.rand(D[0], D[1], D[2], D[3])
            UL = np.random.rand(D[0], D[0]) + 1j*np.random.rand(D[0], D[0])
